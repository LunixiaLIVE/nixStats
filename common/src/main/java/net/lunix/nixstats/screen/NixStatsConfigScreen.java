...
public class NixStatsConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        panelW = Math.min(this.width - 16, Math.max(240, this.width / 3));
        panelX = Math.max(4, (this.width - panelW) / 2);
        innerX = panelX + 6;
        innerW = panelW - 12;

        // Panel anchored near top; bottom buttons anchored to screen bottom
        int panelY = 8;
        boxTop  = panelY;
        buttonsY = this.height - 24;
        ...
        buttonsY          = Math.max(boxBottom + 4, this.height - 24);
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        g.drawCenteredString(font, title, this.width / 2, 4, 0xFFFFFF);
        ...
        g.drawCenteredString(font, "\u2500 Phantom Colors \u2500",
        ...
        g.drawCenteredString(font, "\u2500 Stats \u2500",
        ...
        if (tempStats != null && !tempStats.isEmpty()) {
            ...
            for (int rel = 0; rel < visibleStatRows; rel++) {
                ...
                // ↑ fake button
                boolean upHov = mouseX >= innerX && mouseX < innerX + 14
                             && mouseY >= rowY  && mouseY < rowY + 14;
                g.fill(innerX, rowY, innerX + 14, rowY + 14, upHov ? 0xFF666666 : 0xFF3A3A3A);
                g.drawCenteredString(font, "↑", innerX + 7, rowY + 3,
                        i > 0 ? 0xFFFFFFFF : 0xFF555555);

                // ↓ fake button
                boolean downHov = mouseX >= innerX + 16 && mouseX < innerX + 30
                               && mouseY >= rowY      && mouseY < rowY + 14;
                g.fill(innerX + 16, rowY, innerX + 30, rowY + 14, downHov ? 0xFF666666 : 0xFF3A3A3A);
                g.drawCenteredString(font, "↓", innerX + 23, rowY + 3,
                        i < n - 1 ? 0xFFFFFFFF : 0xFF555555);

                // × fake button
                boolean xHov = mouseX >= innerX + innerW - 14 && mouseX < innerX + innerW
                            && mouseY >= rowY             && mouseY < rowY + 14;
                g.fill(innerX + innerW - 14, rowY, innerX + innerW, rowY + 14,
                        xHov ? 0xFF883333 : 0xFF3A3A3A);
                g.drawCenteredString(font, "×", innerX + innerW - 7, rowY + 3, 0xFFFF5555);
        ...
        if (previewX > panelX + panelW + 8) {
            ...
            g.drawString(font, "Preview:", previewX, previewY - 12, 0xAAAAAA, false);
        ...
        for (int row = 0; row < 3; row++) {
            int labelY = colorRowsBaseY + row * 26;
            g.drawString(font, COLOR_ROW_LABELS[row], innerX, labelY, 0xFFCCCCCC, true);
        }
    }
